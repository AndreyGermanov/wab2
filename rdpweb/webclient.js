...
var RDPWebClient = {
    ...
    {
        if (RDPWebClient.isRDPWebControlById(eventTarget(event).id))
        {
            RDPWebClient._callMouseOut(eventTarget(event).id);
        }
        return true;
    },
    ...
    _MozillaMouse: function(event, fMouseDown)
    {
        if (RDPWebClient.isRDPWebControlById(eventTarget(event).id))
        {
            if (event.button == 2)
            {
                if (fMouseDown)
                {
                    RDPWebClient._callRightMouseDown(eventTarget(event).id);
                }
                else
                {
                    RDPWebClient._callRightMouseUp(eventTarget(event).id);
                }
                RDPWebClient._MozillaCancelEvent(event);
            }
        }
