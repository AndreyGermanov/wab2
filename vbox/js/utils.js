/**
 * $Id: utils.js 360 2011-11-01 18:29:00Z imooreyahoo@gmail.com $
	...
	$(root).find('div.slider').each(function(){
		...
		if($(this).data('display')) {
			...
			$(this).slider('option','slide',function(event,ui){
				document.forms[frm].elements[eventTarget(event).id + 'Value'].value = fn(ui.value);
			}).slider('option','change',function(event,ui){
				document.forms[frm].elements[eventTarget(event).id + 'Value'].value = fn(ui.value);
			});			
		} else {
			$(this).slider('option','slide',function(event,ui){
				document.forms[frm].elements[eventTarget(event).id + 'Value'].value = ui.value;
			}).slider('option','change',function(event,ui){
				document.forms[frm].elements[eventTarget(event).id + 'Value'].value = ui.value;
			});
		}
