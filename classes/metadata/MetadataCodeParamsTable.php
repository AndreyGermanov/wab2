...
class MetadataCodeParamsTable extends DataTable {
    
    function construct($params) {
        parent::construct($params);
        global $Objects;
        $this->usersList = "";
        $this->clientClass = "MetadataCodeParamsTable";
        $this->parentClientClasses = "DataTable~Entity";        
    }

    function getArgs() {
        $result = parent::getArgs();
        global $Objects,$codes;        
        $id = $this->getId();
        $str = "";
		...
		$c=1;
        if (isset($codes[$this->fieldName]) and isset($codes[$this->fieldName]["params"])) {        	
            foreach ($codes[$this->fieldName]["params"] as $key=>$value) {
