<?php
// Справочник форм обращения
class ReferenceRequestForms extends Reference {
	
	function construct($params) {
		parent::construct($params);
		$this->clientClass = "ReferenceRequestForms";
		$this->hierarchy = true;
		$this->parentClientClasses = "Reference~Entity";
		$this->classListTitle = "Формы обращения";
		$this->classTitle = "Форма обращения";		
        ...
        $this->tabset_id = "WebItemTabset_".$this->module_id."_ReferenceRequestForms".$this->name;
	...
	function checkData() {
		...
		if (!$this->isGroup) {
			if ($this->title != "") {				
				...
				if (count($result)>0) {
					$this->reportError("Должность с таким наименованием уже существует","save");
					return 0;
				}
			}
		} 		
