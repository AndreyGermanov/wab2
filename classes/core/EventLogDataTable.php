...
class EventLogDataTable extends DataTable {
    ...
    function construct($params) {
        ...
        $this->clientClass = "EventLogDataTable";
    ...
    function getArgs() {
        ...
        if ($this->loaded!=true) {
            ...
            $adapter = $Objects->get("EventLogDataAdapter_".$this->module_id."_".str_replace($this->module_id."_","",$this->parent_object_id));
            $count = 0;
            ...
            if ($adapter->connected) {
                $stmt = $adapter->dbh->prepare(str_replace("null","","SELECT count(eventDate) as cnt FROM ".$this->table." ".$this->condition." ".$this->sortOrder));
	        $stmt->execute();
            ...
            if ($limit!="")
                $limit = "LIMIT ".$limit;
            if ($adapter->connected) {
                $stmt = $adapter->dbh->prepare(str_replace("null","","SELECT * FROM ".$this->table." ".$this->condition." ".$this->sortOrder." ".$limit));
	        	$stmt->execute();
                ...
                foreach ($fieldList as $field) {
                    ...
                    if ($field_parts[0]!="") {
                        if ($field_parts[0]=="eventDate") {
                            ...
                            $report = $Objects->get("EventLog_".$this->module_id."_Events");
