var MetadataRole = Class.create(Entity, {
            ...
            onSuccess: function(transport)
            {
                ...
                if (response!="")
                {
	                	...
	                	for (i=0;i<elems.length;i++) {
	                		...
	                		if (elem.id!=null) {
	                			...
	                			if (elem_end==obj.old_name) {
	                                ...
	                                params["sorting"] = "none";
	                                var icon = obj.skinPath+"images/Tree/role.gif";
	                                params["image"] = icon;
	                                if (data["old_name"]=="") {
	                                    params["action"] = "add";
                	...
                	if (obj.old_name=="") {
                        ...
                        var new_owner_id = old_owner_id;
                        var old_id = "MetadataRole_"+obj.module_id+"_"+obj.rnd+"_"+data["name"];
                        var new_id = old_id;
                        var new_target_id = new_id;
                        var params = new Array;
                        if (obj.opener_item.parentNode !=null)
    ...
    changeProfileBtn_onClick: function(event) {
    	var elem_id = $O(this.object_id+"_profilesList","").getValue();
		var wm = getWindowManager();		
    ...
    removeProfileBtn_onClick: function(event) {
        if (confirm("Вы действительно хотите удалить этот профиль ?")) {
	    	var list = $I(this.object_id+"_profilesList_value");
	    	if (list.selectedIndex!=-1) {
		    	var elem_id = $O(this.object_id+"_profilesList","").getValue();
		    	elem_id = elem_id.replace(this.module_id+"_","");
		    	var arr = elem_id.split("_");
		    	arr.shift();
		    	arr.shift();
		    	elem_id = arr.join("_");
		    	var args = new Object;
		    	args["profile"] = elem_id;
		        new Ajax.Request("index.php", {
		            method:"post",
    ...
    NODE_CLICKED_processEvent: function(params) {
    	if (params["result_object_id"]==this.object_id) {
    		var elem_id = "";
    		if (params["node_id"]=="") {
    			elem_id = params["node_class"]+"_"+this.module_id+"_";
    		} else
    			elem_id = params["node_id"];
	    	var obj = this;
	        new Ajax.Request("index.php", {
	            method:"post",
	            parameters: {ajax: true, object_id: elem_id, hook: 'getProfileClass'},
	            onSuccess: function(transport) {
	            	var response = transport.responseText;
	            	if (response!="") {
	            		var arr = elem_id.split("_");
	            		var objClass = arr.shift();
	            		elem_id = arr.join("_").replace(obj.module_id+"_",""); 
	            		elem_id = response+"_"+obj.module_id+"_"+obj.name+"_"+objClass+"_"+elem_id;
	            		var window_elem_id = "Window_"+elem_id.replace(/_/g,"");
	            		var params = new Object;
	            		var wm = getWindowManager();		
	            		wm.show_window(window_elem_id,elem_id,params,obj.object_id,obj.node.id,null,false);		    	
	            	}	            
	            }
	        });
    	}
    ...
    ROLE_PROFILE_CHANGED_processEvent: function(params) {
    	if (params["parent_object_id"]==this.object_id) {
	    	...
	    	for (o in list.options) {
	    		...
	    		if (option.value==params["profileId"]) {
	    			found = true;
	    			break;
	    		}
	    	}
	    	if (!found) {
	    		list.options[list.options.length] = new Option(params["profileTitle"],params["profileId"],false,false);
	    	}
    	}
    }
});