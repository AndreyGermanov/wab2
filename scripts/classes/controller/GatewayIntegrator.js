var GatewayIntegrator = Class.create(Mailbox, {
    ...
    OK_onClick: function(event) {        
        ...
        if (is_changed =="true")
        {
            ...
            if (!check_ip(data["gatewayIp"])) {
                this.reportMessage('Ip-адрес Интернет-шлюза указан неверно!',"error",true);
                return 0;
            }
            var params = data.toObject();
            
            var objectid = "GatewayIntegrator_"+this.module_id+"_Mail";
            var mbox = this;
