...
document.write('<div style="z-index:5"><table id="fc" style="position:absolute;border-collapse:collapse;background:#FFFFFF;border:1px solid #303030;display:none;-moz-user-select:none;-khtml-user-select:none;user-select:none;z-index:5" cellpadding=2>');
...
function cs_click(e) {
		var arr = updobj.value.split(" ");
		var time = "";
		if (arr.length==2)
			time = " "+updobj.value.split(" ").pop();
        updobj.value=calvalarr[evtTgt(EvtObj(e)).id.substring(1,evtTgt(EvtObj(e)).id.length)]+time;
        if (updobj.getAttribute("object")!=null) {
            $O(updobj.getAttribute("object"),'').changeDateField(e);
        }           
	getObj('fc').style.display='none';
        return false;
}
...
function today() {
	...
	if (arr.length==2)
		time = " "+updobj.value.split(" ").pop();	
	updobj.value=addnull(now.getDate(),now.getMonth()+1,now.getFullYear())+time;
	getObj('fc').style.display='none';
