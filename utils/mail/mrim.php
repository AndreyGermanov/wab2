 ...
 * @version    3.0 build 1020
 * @link       http://tem.dp.ua/releases/Eugen/MRIM.PHP/v3/class.mrim.phps
...
class mrim
{
    ...
    function connect_socks4_proxy($proxy,$port,$user = null)
    {
        ...
        if(!$this->sock)
            ...
            $request = pack('C2',0x04,0x01).pack('n',$this->port).$int.($user)?$user:'0'.pack('C',0x00);
            ...
            if(!$this->sock)
                ...
                $request = pack('C2',0x04,0x01).pack('n',$portc).$int.($user)?$user:'0'.pack('C',0x00);
                fwrite($this->sock,$request);
                $resp = fread($this->sock,9);
                $answer = unpack('Cvn/Ccd',substr($resp,0,2));
                if($answer['vn'] != 0x00) {
    ...
    function connect_socks5_proxy($proxy,$port,$user = null,$password = null)
    {
        ...
        if(!$this->sock)
            ...
            if($answer['method'] == 0x02) {
                $request = pack('C',0x01).pack('C',strlen($user)).$user.pack('C',strlen($password)).$password;
                fwrite($this->sock,$request);
                $resp = fread($this->sock,3);
            ...
            if($answer['method'] == 0x00) {
                ...
                $request = pack('C4',0x05,0x01,0x00,0x01).$int.pack('n',$this->port);
                fwrite($this->sock,$request);
                ...
                if(!$this->sock)
                    ...
                    if($answer['method'] == 0x00) {
                        ...
                        $request = pack('C4',0x05,0x01,0x00,0x01).$int.pack('n',$portc);
                        fwrite($this->sock,$request);
                        $resp = fread($this->sock,11);
                        $answer = unpack('Cver/CREP',substr($resp,0,2));
                        if($answer['REP'] != 0x00) {
    ...
    function login($login,$pass)
    {
        if(!$this->is_connected())
            return false;
        $user_agent = 'MRIM.php/3.0 (PRO; '.PHP_OS.'; Proxy)';
        $user_agent_descr = 'client="MRIM.php" version="3.0" build="1020"';
        $st = 'STATUS_INVISIBLE';
        $this->status = 'invisible';
        $lang = 'ru';
        $login_data = pack('L1',strlen($login)).$login.pack('L1',strlen($pass)).$pass.pack('L2',$this->STATUS_ONLINE | $this->STATUS_FLAG_INVISIBLE,strlen($st)).$st.pack('L4',0x00,0x00,0x03FF,strlen($user_agent_descr)).$user_agent_descr.pack('L1',strlen($lang)).$lang.pack('L1',strlen($user_agent)).$user_agent;
    ...
    function message($to,$text)
    {
        ...
        $data = pack('L2',$this->MESSAGE_FLAG_OFFLINE,strlen($to)).$to.pack('L1',strlen($text)).$text.pack('L1',0);
    ...
    function message_alarm($to)
    {
        ...
        $rtext = pack('L2',2,strlen($rtext)).$rtext.pack('L2',4,0xFFFFFF);
        $rtext = base64_encode(gzcompress($rtext,9));
        $data = pack('L2',$this->MESSAGE_FLAG_ALARM | $this->MESSAGE_FLAG_RTF,strlen($to)).$to.pack('L1',strlen($text)).$text.pack('L1',strlen($rtext)).$rtext;
        fwrite($this->sock,$this->make_packet($this->MRIM_CS_MESSAGE,$data));
        return true;
    }
    /**
    ...
    function message_notify($to)
    {
        if(!$this->is_connected())
            return false;
        $data = pack('L2',$this->MESSAGE_FLAG_NOTIFY,strlen($to)).$to.pack('L1',1).' '.pack('L1',1).' ';
        fwrite($this->sock,$this->make_packet($this->MRIM_CS_MESSAGE,$data));
        return true;
    }
    /**
    ...
    function message_contact($to,$text)
    {
        ...
        $data = pack('L2',$this->MESSAGE_FLAG_CONTACT,strlen($to)).$to.pack('L1',strlen($texts)).$texts.pack('L1',0);
        fwrite($this->sock,$this->make_packet($this->MRIM_CS_MESSAGE,$data));
        return true;
    }
    /**
    ...
    function sms($to,$text)
    {
        if(!$this->is_connected())
            return false;
        $text = iconv('UTF-8','UTF-16LE//IGNORE',$text);
        $data = pack('L2',0,strlen($to)).$to.pack('L1',strlen($text)).$text;
    ...
    function authorize($email)
    {
        if(!$this->is_connected())
            return false;
        $data = pack('L1',strlen($email)).$email;
    ...
    function set_status($status = 'invisible',$xstatus = '',$xstatus_text = '')
    {
        ...
        $data = pack('L2',$status,strlen($status2)).$status2.pack('L1',strlen($xstatus)).$xstatus.pack('L1',strlen($xstatus_text)).$xstatus_text.pack('L1',0x03FF);
    ...
    function add_contact($addr,$name,$text = 'Здравствуйте. Пожалуйста, добавьте меня в список ваших контактов.',$phone = '',$group = 0,$invisible = false,$visible = false,$ignore = false,$shadow = false,$deleted = false,$sms = false)
    {
        ...
        $text = iconv('UTF-8','UTF-16LE//IGNORE',$text);
        $text = base64_encode(pack('L2',2,strlen($nickname)).$nickname.pack('L1',strlen($text)).$text);
        $data = pack('L3',$flags,$group,strlen($addr)).$addr.pack('L1',strlen($name)).$name.pack('L1',strlen($phone)).$phone.pack('L1',strlen($text)).$text.pack('L1',0);
    ...
    function add_group($name)
    {
        ...
        $name = iconv('UTF-8','UTF-16LE//IGNORE',$name);
        $nickname = iconv('UTF-8','UTF-16LE//IGNORE',$nickname);
        $text = base64_encode(pack('L2',2,strlen($nickname)).$nickname.pack('L1',0));
        $data = pack('L4',$flags,0,0,strlen($name)).$name.pack('L2',0,strlen($text)).$text.pack('L1',0);
        fwrite($this->sock,$this->make_packet($this->MRIM_CS_ADD_CONTACT,$data));
        return true;
    }
    /**
    ...
    function modify_group($id,$name,$shadow = false,$deleted = false)
    {
        ...
        if($shadow)
            $flags |= $this->CONTACT_FLAG_SHADOW;
        $name = iconv('UTF-8','UTF-16LE//IGNORE',$name);
        $data = pack('L5',$id,$flags,0,0,strlen($name)).$name.pack('L1',0);
    ...
    function modify_contact($id,$addr,$name,$group,$phone = '',$invisible = false,$visible = false,$ignore = false,$shadow = false,$deleted = false,$sms = false)
    {
        ...
        $name = iconv('UTF-8','UTF-16LE//IGNORE',$name);
        $data = pack('L4',$id,$flags,$group,strlen($addr)).$addr.pack('L1',strlen($name)).$name.pack('L1',strlen($phone)).$phone;
        fwrite($this->sock,$this->make_packet($this->MRIM_CS_MODIFY_CONTACT,$data));
        return true;
    }
    /**
    ...
    function find_contact($addr = '',$nickname = '',$name = '',$lastname = '',$sex = false,$min = '',$max = '',$city = '',$zodiac = '',$month = '',$day = '',$country = '',$online = false)
    {
        ...
        if($addr) {
            ...
            $data .= pack('L2',$this->MRIM_CS_WP_REQUEST_PARAM_USER,strlen($n)).$n;
            $data .= pack('L2',$this->MRIM_CS_WP_REQUEST_PARAM_DOMAIN,strlen($d)).$d;
        }
        if($nickname) {
            $nickname = iconv('UTF-8','UTF-16LE//IGNORE',$nickname);
            $data .= pack('L2',$this->MRIM_CS_WP_REQUEST_PARAM_NICKNAME,strlen($nickname)).$nickname;
        }
        if($name) {
            $name = iconv('UTF-8','UTF-16LE//IGNORE',$name);
            $data .= pack('L2',$this->MRIM_CS_WP_REQUEST_PARAM_FIRSTNAME,strlen($name)).$name;
        }
        if($lastname) {
            $lastname = iconv('UTF-8','UTF-16LE//IGNORE',$lastname);
            $data .= pack('L2',$this->MRIM_CS_WP_REQUEST_PARAM_LASTNAME,strlen($lastname)).$lastname;
        }
        if(!is_bool($sex))
            $data .= pack('L2',$this->MRIM_CS_WP_REQUEST_PARAM_SEX,1).($sex == 2?2:1);
        if($min)
            $data .= pack('L2',$this->MRIM_CS_WP_REQUEST_PARAM_DATE1,strlen($min)).$min;
        if($max)
            $data .= pack('L2',$this->MRIM_CS_WP_REQUEST_PARAM_DATE2,strlen($max)).$max;
        if(strlen($city))
            $data .= pack('L2',$this->MRIM_CS_WP_REQUEST_PARAM_CITY_ID,strlen($city)).$city;
        if($zodiac)
            $data .= pack('L2',$this->MRIM_CS_WP_REQUEST_PARAM_ZODIAC,strlen($zodiac)).$zodiac;
        if($month)
            $data .= pack('L2',$this->MRIM_CS_WP_REQUEST_PARAM_BIRTHDAY_MONTH,strlen($month)).$month;
        if($day)
            $data .= pack('L2',$this->MRIM_CS_WP_REQUEST_PARAM_BIRTHDAY_DAY,strlen($day)).$day;
        if(strlen($country))
            $data .= pack('L2',$this->MRIM_CS_WP_REQUEST_PARAM_COUNTRY_ID,strlen($country)).$country;
        if($online)
            $data .= pack('L2',$this->MRIM_CS_WP_REQUEST_PARAM_ONLINE,1).'1';
    ...
    function is_filetransfer_status($packet)
    {
        ...
        if($packet[0] != $this->MRIM_CS_FILE_TRANSFER_ACK)
            return false;
        list(,$status,$len) = unpack('L2',$packet[1]);
        ...
        list(,$id,$len) = unpack('L2',$packet[1]);
        $ret['id'] = $id;
        $packet[1] = substr($packet[1],8);
        if($len) {
    ...
    function is_filetransfer($packet)
    {
        ...
        $ret['email'] = substr($packet[1],0,$len);
        $packet[1] = substr($packet[1],$len);
        list(,$id,$size,$len) = unpack('L3',$packet[1]);
        $ret['id'] = $id;
        $ret['size'] = $size;
    ...
    function is_session_key($packet)
    {
        ...
        if($packet[0] != $this->MRIM_CS_MPOP_SESSION)
            return false;
        list(,$success,$len) = unpack('L2',$packet[1]);
    ...
    function is_new_ping($packet)
    {
        ...
        if($packet[0] != $this->MRIM_CS_CONNECTION_PARAMS)
            return false;
        list(,$newping) = unpack('L1',$packet[1]);
    ...
    function is_logout($packet)
    {
        ...
        if($packet[0] != $this->MRIM_CS_LOGOUT)
            return false;
        list(,$reason) = unpack('L1',$packet[1]);
    ...
    function is_contact_list($packet)
    {
        ...
        if($status != $this->GET_CONTACTS_OK)
            return false;
        list(,$groups) = unpack('L1',$packet[1]);
        $packet[1] = substr($packet[1],4);
        list(,$len) = unpack('L1',$packet[1]);
        $packet[1] = substr($packet[1],4);
        ...
        while(strlen($packet[1]) > 0) {
            ...
            if($contact_mask[3] == 's') {
                ...
                return false;
            if($contact_mask[4] == 'u') {
                list(,$sflags) = unpack('L1',$packet[1]);
                $packet[1] = substr($packet[1],4);
            } else
                return false;
            if($contact_mask[5] == 'u') {
