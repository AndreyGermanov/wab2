...
class Mrim:
  ...
  def connect(self, proxy = None, port = None, user = None, password = None, type = None):
    self.proxy = None  
  ...
  def connect_single(self):
    ...
    data = s.recv(48)
    list = struct.unpack('<12L', data)
    self.ping_time = list[-1]
    self.sock = s
    return True
  ...
  def set_status(self, status = "invisible", xstatus = '', xstatus_text = ''):
    ...
    data = struct.pack('<L', status) + self.pack_data(status2) + self.pack_data(xstatus, 'UTF-16LE') + self.pack_data(xstatus_text, 'UTF-16LE') + struct.pack('<L', 0x03FF)
  ...
  def login(self, login, password):
    ...
    login_data = self.pack_data(login) + self.pack_data(password) + struct.pack('<L', self.STATUS_ONLINE | self.STATUS_FLAG_INVISIBLE) + self.pack_data(st) + struct.pack('<3L', 0x00, 0x00, 0x03FF) + self.pack_data(self.user_agent_descr) + self.pack_data(lang) + self.pack_data(self.user_agent)
    self.sock.send(self.make_packet(self.MRIM_CS_LOGIN2, login_data))
    data = self.sock.recv(44)
    list = struct.unpack('<11L', data)
    msg = list[3]
  ...
  def is_new_ping(self, packet):
    ...
    list = struct.unpack('<L', packet[1])
    self.ping_time = list[0]
    return True
  ...
  def add_contact(self, addr, name, text = "Здравствуйте. Пожалуйста, добавьте меня в список ваших контактов. ", phone = "", group = 0, invisible = False, visible = False, ignore = False, shadow = False, deleted = False, sms = False):
    if not self.is_connected():
      return False
    ...
    txt = base64.b64encode(struct.pack('<L', 2) + self.pack_data(nickname, 'UTF-16LE') + self.pack_data(text, 'UTF-16LE'))
    #data = struct.pack('<2L', flags, group) + self.pack_data(addr) + self.pack_data(name) + self.pack_data(phone) + struct.pack('<L', len(txt)) + txt + struct.pack('<L', 0)
    data = struct.pack('<2L', flags, group) + self.pack_data(addr) + self.pack_data(name, 'UTF-16LE') + self.pack_data(phone) + struct.pack('<L', len(txt)) + txt + struct.pack('<L', 0)
    #+ struct.pack('<L', 0)
    #+ struct.pack('<L', len(txt)) + txt + struct.pack('<L', 0)
